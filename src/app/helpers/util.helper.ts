...
export class Util {
  ...
  static async openDefaultDialog(config?: any) {
    if(!config) config = {width: '250px', height:'250px', data: { title: 'Alert', body: 'Internal Server Error' }};
  ...
  static TE = function(err_message: string, log?: boolean) { // TE stands for Throw Error
    ...
    // const config = {width: '250px', height: '280px', data: { title: 'Alert!', body: err_message }};
    const config = {data: { title: 'Alert!', body: err_message }};
    this.openDefaultDialog(config);
  ...
  static async delete(url, data?: any) {
    ...
    let err, res;
    [err, res] = await this.to(this.http.delete(url, { headers: headers, body: data }).toPromise(), false);
    return this.responder(err, res);
  }
